...
namespace EU.CqrXs.Gui.Forms
{
    ...
    public partial class EncryptFormMultiControls : EncryptFormBase
    {
        ...
        protected internal void pictureBoxAddAlgo_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(comboBoxAlgo.SelectedItem.ToString()) && Enum.TryParse<CipherEnum>(comboBoxAlgo.SelectedItem.ToString(), out CipherEnum cipherEnum))
            {
                if (cipherAlgos.Length < Constants.PIPE_MAX_LEN)
                {
                    ...
                    cPipe = new CipherPipe(cipherAlgos, Constants.PIPE_MAX_LEN, GetEncoding(), GetZip(), GetHash(), GetCipherMode2());
        ...
        protected internal async Task Encrypt_Click(object sender, EventArgs e)
        {
            ...
            cPipe = new CipherPipe(pipeAlgos, Constants.PIPE_MAX_LEN, GetEncoding(), GetZip(), GetHash(), GetCipherMode2());
            ...
            if (!string.IsNullOrEmpty(groupBoxFiles.labelFileIn.Text) && !groupBoxFiles.labelFileIn.Text.StartsWith("["))
            {
                ...
                try
                {
                    ...
                    string outFilePath = (fileName + GetCipherMode2().GetCipherMode2Extension() + GetHash().GetExtension() + GetZip().GetZipTypeExtension() + miniPipe + GetEncoding().GetEnCodingExtension());
        ...
        internal void menuMainItemZenMatrixTest_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                HideAllOtherForms("ZenMatrixForm");
            }
            catch (Exception exHide)
            {
        ...
        internal void menuMainAsymmetric_Click(object sender, EventArgs e)
        {
            ...
            {
                Area23Log.LogOriginEx("EncryptFormMultiControls.menuMainFormAsymmetric_Click", exShow, 1);
                Program.formAsymmetric = new EncryptFormAsymmetric();
                Program.formAsymmetric.Show();
            }
            try
